...
#### `fixed` 모드 — 평단 분할익절 + 다중 포지션
| 트리거 | 동작 |
|--------|------|
| 최초 익절 | 봇 보유분 가중평단 대비 +3% 이상 → 전체의 30% 매도 |
| 이후 익절 | 마지막 매도 24시간 후에도 +3% 이상이면 잔량의 30%, +6% 이상이면 60%, +9% 이상이면 전량 매도 |
| 매도 쿨다운 | 체결 시점부터 24시간 동안 추가 매도 없음 (종목별 독립) |
| 손절 / 트레일링 / 추세이탈 | **없음** (손실 중에는 계속 보유) |
...
> ℹ️ `fixed` 모드의 평단은 baseline으로 보호된 기존 자산을 제외하고, 봇이 매수한 잔량만으로 계산합니다. 최초 매도는 수익률이 6%나 9%를 넘었더라도 30%이며, 그다음 매도부터 9% → 6% → 3% 순으로 가장 높은 조건을 적용합니다.
